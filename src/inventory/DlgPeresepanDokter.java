...
//            String raw = LTotal.getText().split(",")[0];  
//            int total = Integer.parseInt(raw.replace(".", ""));
            String cleaned = LTotal.getText().replaceAll("[.,]", "");
            int total = Integer.parseInt(cleaned);

//            int total = Integer.parseInt(LTotal.getText().replace(".", ""));
            if(total >= 60000){
                int warningObat = JOptionPane.showConfirmDialog(rootPane,"Obat sudah Melebihi dari 60000? Anda yakin?","Konfirmasi",JOptionPane.YES_NO_OPTION);
                
                if(warningObat==JOptionPane.YES_OPTION){

...
public final class DlgResepObat extends javax.swing.JDialog {
    ...
    private String now = dateFormat.format(date), lembarobat = "", status = "", rincianobat = "", finger = "", ptTelaah = "", nmTelaah = "";
    ...
    private void initComponents() {
        ...
        BtnTelaah1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/report24.png"))); // NOI18N
        BtnTelaah1.setMnemonic('U');
        BtnTelaah1.setText("Cetak");
        BtnTelaah1.setToolTipText("Alt+U");
        BtnTelaah1.setName("BtnCetakTelaah"); // NOI18N
        BtnTelaah1.setPreferredSize(new java.awt.Dimension(100, 30));
        BtnTelaah1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnCetakTelaahActionPerformed(evt);
            }
        });
        FormPass3.add(BtnTelaah1);
        BtnTelaah1.getAccessibleContext().setAccessibleName("CetakTelaah");
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (Sequel.cariInteger(
                ...
                Valid.MyReportqry("rptItemResep.jasper", "report", "::[ Aturan Pakai Obat ]::",
                        ...
                        + "aturan_pakai.aturan,detail_pemberian_obat.jml,kodesatuan.satuan,pasien.jk,reg_periksa.umurdaftar,reg_periksa.sttsumur,DATE_FORMAT(databarang.expire, '%d-%m-%Y') AS expire "
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (Sequel.cariInteger(
                ...
                Valid.MyReportqry("rptItemResep5.jasper", "report", "::[ Aturan Pakai Obat ]::",
                        "select resep_obat.no_resep,resep_obat.tgl_perawatan,resep_obat.jam,pasien.tgl_lahir, "
                        + "resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,databarang.nama_brng,"
                        + "aturan_pakai.aturan,detail_pemberian_obat.jml,kodesatuan.satuan,pasien.jk,reg_periksa.umurdaftar,reg_periksa.sttsumur,DATE_FORMAT(databarang.expire, '%d-%m-%Y') AS expire, IFNULL(bangsal.nm_bangsal, poliklinik.nm_poli) AS nm_bangsal "
                        + "from resep_obat inner join reg_periksa inner join pasien inner join "
                        + "aturan_pakai inner join databarang inner join detail_pemberian_obat "
                        + "inner join kodesatuan on resep_obat.no_rawat=reg_periksa.no_rawat  "
                        + "and reg_periksa.no_rkm_medis=pasien.no_rkm_medis and "
            ...
            if (Sequel.cariInteger(
                ...
                Valid.MyReportqry("rptItemResep6.jasper", "report", "::[ Aturan Pakai Obat ]::",
                        "select resep_obat.no_resep,resep_obat.tgl_perawatan,resep_obat.jam,pasien.tgl_lahir,"
                        + "resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,obat_racikan.nama_racik,"
                        + "obat_racikan.aturan_pakai,obat_racikan.jml_dr,metode_racik.nm_racik,pasien.jk,reg_periksa.umurdaftar,reg_periksa.sttsumur, IFNULL(bangsal.nm_bangsal, poliklinik.nm_poli) AS nm_bangsal"
                        + "from resep_obat inner join reg_periksa inner join pasien inner join "
                        + "obat_racikan inner join metode_racik on resep_obat.no_rawat=reg_periksa.no_rawat "
                        + "and reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "and obat_racikan.kd_racik=metode_racik.kd_racik "
...
//                    "select detail_pemberian_obat.tgl_perawatan,detail_pemberian_obat.jam,databarang.kode_sat, "+
//                    "detail_pemberian_obat.kode_brng,detail_pemberian_obat.jml,detail_pemberian_obat.total,"+
//                    "databarang.nama_brng,detail_pemberian_obat.no_rawat,resep_obat.no_resep from detail_pemberian_obat inner join databarang "+
//                    "on detail_pemberian_obat.kode_brng=databarang.kode_brng inner join resep_obat on resep_obat.no_rawat=detail_pemberian_obat.no_rawat "+
//                    "and detail_pemberian_obat.tgl_perawatan=resep_obat.tgl_perawatan and detail_pemberian_obat.jam=resep_obat.jam "+
//                    "where resep_obat.no_resep=?");
                ps = koneksi.prepareStatement(
                        "SELECT resep_dokter.no_resep, resep_dokter.kode_brng, resep_dokter.jml, resep_dokter.aturan_pakai, databarang.nama_brng "
                        + "FROM resep_dokter INNER JOIN databarang ON databarang.kode_brng = resep_dokter.kode_brng "
                ...
                try {
                    ...
                    while (rs.next()) {
                        Sequel.menyimpan("temporary_resep", "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", 38, new String[]{
                            "" + i, rs.getString("nama_brng"), rs.getString("aturan_pakai"), rs.getString("jml"), "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", akses.getalamatip()
                        });
                        i++;
                    }
                } catch (Exception e) {
...
//            }
    ...
    private void BtnCetakTelaahActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCetakTelaahActionPerformed
        ...
        if (NoResep.getText().equals("")) {
            ...
            finger = Sequel.cariIsi("select sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?", ptTelaah);
            param.put("finger", "Dikeluarkan di " + akses.getnamars() + ", Kabupaten/Kota " + akses.getkabupatenrs() + "\nDitandatangani secara elektronik oleh " + nmTelaah + "\nID " + (finger.equals("") ? ptTelaah : finger) + "\n" + DTPBeri.getSelectedItem());
            param.put("jam", cmbJam.getSelectedItem() + ":" + cmbMnt.getSelectedItem() + ":" + cmbDtk.getSelectedItem());
            param.put("logo", Sequel.cariGambar("select setting.logo from setting"));
            Valid.MyReportqry(
                    "rptCetakTelaahFarmasi.jasper",
                    "report",
                    "::[ Telaah Farmasi ]::",
                    "SELECT "
                    ...
                    + "FROM telaah_farmasi "
                    + "left JOIN petugas ON telaah_farmasi.nip = petugas.nip "
                    + "WHERE telaah_farmasi.no_resep='"
                    + NoResep.getText() + "'",
                    param
            );
        }

    }//GEN-LAST:event_BtnCetakTelaahActionPerformed
